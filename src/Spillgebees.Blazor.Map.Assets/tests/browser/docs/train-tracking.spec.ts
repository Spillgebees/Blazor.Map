...
test.describe
  .skip("train-tracking (disabled pending docs site selector update)", () => {
    ...
    function buildToggleSnapshotExpectation(
      overrides: Partial<ToggleVisibilitySnapshot> = {},
    ...
    const _trainShowcaseMapSelector = ".train-showcase-map .sgb-map-container";
    ...
    async function mockLuxembourgOverlayStyle(page: Page): Promise<void> {
      ...
      await page.route(/.*(?:style\.json)$/, async (route) => {
        ...
        // act
        if (
          requestUrl.includes("openfreemap.org") ||
          requestUrl.includes("openmaptiles") ||
          requestUrl.includes("maptiler") ||
          requestUrl.includes("tiles.spillgebees.dev")
        ) {
    ...
    async function readTrainShowcaseLayerSnapshot(page: Page): Promise<TrainShowcaseLayerSnapshot> {
      return page.evaluate<TrainShowcaseLayerSnapshot>(() => {
        ...
        const overlayStationLayerId =
          orderedLayerIds.find((layerId) => layerId.includes("railway-stations-circle")) ?? null;
        const overlayStationLabelLayerId =
          orderedLayerIds.find((layerId) => layerId.includes("railway-stations-label")) ?? null;
        ...
        return {
          ...
          overlayStationCount:
            overlayStationLayerId === null
              ? 0
              : map.queryRenderedFeatures(bounds, { layers: [overlayStationLayerId] }).length,
          ...
          serviceFeatureCountInSource: Array.from(
            (
              sourceRegistrations?.get("train-source-decorations")?.sourceSpec.data as {
    ...
    async function readToggleVisibilitySnapshot(page: Page): Promise<ToggleVisibilitySnapshot> {
      return page.evaluate<ToggleVisibilitySnapshot>(() => {
        ...
        return {
          tracksVisibility: map.getLayoutProperty(
            "sgb-overlay-style-sgb-train-tracking-overlay-railway-line-rail",
            "visibility",
          ),
          tramVisibility: map.getLayoutProperty(
            "sgb-overlay-style-sgb-train-tracking-overlay-tram-line-fill",
            "visibility",
          ),
          stationsVisibility: map.getLayoutProperty(
            "sgb-overlay-style-sgb-train-tracking-overlay-railway-stations-circle",
            "visibility",
          ),
          platformsVisibility: map.getLayoutProperty(
            "sgb-overlay-style-sgb-train-tracking-overlay-railway-platforms-fill",
            "visibility",
          ),
          routesVisibility: map.getLayoutProperty(
            "sgb-overlay-style-sgb-train-tracking-overlay-railway-routes",
            "visibility",
          ),
          lifecycleVisibility: map.getLayoutProperty(
            "sgb-overlay-style-sgb-train-tracking-overlay-railway-lifecycle-fill",
            "visibility",
          ),
          infrastructureVisibility: map.getLayoutProperty(
            "sgb-overlay-style-sgb-train-tracking-overlay-railway-switches",
            "visibility",
          ),
          trainClusterVisibility: map.getLayoutProperty("train-source-clusters", "visibility"),
        };
    ...
    async function _readProjectedLayerFeaturePoint(page: Page, layerId: string): Promise<ProjectedLayerFeaturePoint> {
    ...
    async function readTrainInteractionSnapshot(page: Page): Promise<TrainInteractionSnapshot> {
      return page.evaluate<TrainInteractionSnapshot>(() => {
        ...
        const decorationSourceRegistration = window.Spillgebees.Map.sourceSpecs
          .get(map)
          ?.get("train-source-decorations");
        ...
        const entityIds = new Set(
          decorationFeatures.map((f) => f.properties?.entityId).filter((id): id is string => typeof id === "string"),
        );
        ...
        const hoveredRouteLabelCount = new Set(
          decorationFeatures
            .filter(
              (f) =>
                f.properties?.decorationId === "route" &&
                typeof f.properties?.entityId === "string" &&
                hoveredEntityIds.has(f.properties.entityId as string),
            )
        ...
        const hoveredOperatorLabelCount = new Set(
          decorationFeatures
            .filter(
              (f) =>
                f.properties?.decorationId === "operator" &&
                typeof f.properties?.entityId === "string" &&
                hoveredEntityIds.has(f.properties.entityId as string),
            )
            .map((f) => f.properties?.entityId),
          ...
          decorationFeatures
            .filter(
              (f) =>
                f.properties?.decorationId === "operator" &&
                typeof f.properties?.entityId === "string" &&
                selectedEntityIds.has(f.properties.entityId as string),
            )
            .map((f) => f.properties?.entityId),
        ).size;
        return {
          ...
          operatorLabelCount: map.queryRenderedFeatures(
            [
              [0, 0],
          ...
          routeLabelCount: map.queryRenderedFeatures(
            [
              [0, 0],
              [map.getCanvas().clientWidth, map.getCanvas().clientHeight],
            ],
          ...
          selectedTrainPanelVisible: Array.from(document.querySelectorAll(".train-card-label")).some(
            (element) => element.textContent?.includes("Selected train") ?? false,
          ),
          zoom: map.getZoom(),
        };
      });
    ...
    async function ensureOverlayGroupTogglesReady(page: Page): Promise<void> {
      await expect
        .poll(async () => readToggleVisibilitySnapshot(page), { timeout: 20_000 })
        .toEqual(buildToggleSnapshotExpectation());
    }
    ...
    async function setLegendToggleChecked(toggle: ReturnType<Page["locator"]>, checked: boolean): Promise<void> {
      ...
      await label.evaluate((element, nextChecked) => {
        // biome-ignore lint/security/noSecrets: CSS selector, not a credential
    ...
    test.describe("train tracking showcase", () => {
      ...
      test("should keep showcase toggles wired to live layer visibility", async ({ page }) => {
        ...
        await expect
          .poll(async () => readToggleVisibilitySnapshot(page))
          .toEqual({
            tracksVisibility: "visible",
            tramVisibility: "none",
            stationsVisibility: "visible",
            ...
            () =>
              page.evaluate(() => {
                const mapContainer = document.querySelector(
                  ".train-showcase-map .sgb-map-container",
                ) as HTMLElement | null;
                const legend = document.querySelector(".sgb-map-legend") as HTMLElement | null;
                const panel = legend?.querySelector(".sgb-map-legend-panel") as HTMLElement | null;
                const body = legend?.querySelector(".sgb-map-legend-body") as HTMLElement | null;
      ...
      test("should render Luxembourg legend swatches with the richer live-style structure", async ({ page }) => {
        ...
        // act
        const snapshot = await page.evaluate(() => {
          const trackSwatch = document.querySelector(".train-overlay-swatch-tracks") as HTMLElement | null;
          const infrastructureSwatch = document.querySelector(
            ".train-overlay-swatch-infrastructure",
          ) as HTMLElement | null;
          const stationSwatch = document.querySelector(".train-overlay-swatch-stations") as HTMLElement | null;
          const tramSwatch = document.querySelector(".train-overlay-swatch-tram") as HTMLElement | null;
          const lifecycleSwatch = document.querySelector(".train-overlay-swatch-lifecycle") as HTMLElement | null;
      ...
      test("should toggle all mapped overlay layers for every overlay group", async ({ page }) => {
        ...
        for (const group of overlayGroups) {
          ...
          const toggledState = Object.fromEntries(
            group.keys.map((key) => [key, group.checkedByDefault ? "none" : "visible"]),
          ) as Partial<ToggleVisibilitySnapshot>;
          const restoredState = Object.fromEntries(
            group.keys.map((key) => [key, group.checkedByDefault ? "visible" : "none"]),
          ) as Partial<ToggleVisibilitySnapshot>;
          ...
          await expect
            .poll(async () => readToggleVisibilitySnapshot(page))
            .toEqual(buildToggleSnapshotExpectation(toggledState));
          ...
          await expect
            .poll(async () => readToggleVisibilitySnapshot(page))
            .toEqual(buildToggleSnapshotExpectation(restoredState));
        }
      });
      ...
      test("should hide train number labels while clustering is active", async ({ page }) => {
        ...
        await expect
          .poll(async () => (await readRenderedFeatureSnapshot(page)).clusterCount, { timeout: 20_000 })
          .toBeGreaterThan(0);
      ...
      test("should render the train number label above and away from the train icon when unclustered", async ({
        page,
      }) => {
        // arrange
        await mockLuxembourgOverlayStyle(page);
      ...
      test("should hover a rendered train and show hover-driven behavior", async ({ page }) => {
        ...
        // assert
        await expect
          .poll(async () => (await readTrainInteractionSnapshot(page)).hoveredDecorationCount, { timeout: 20_000 })
          .toBeGreaterThan(0);
      ...
      test("should click a rendered train and keep selection after the next update", async ({ page }) => {
        ...
        // assert
        await expect
          .poll(() => readTrainInteractionSnapshot(page), { timeout: 20_000 })
          .toMatchObject({
      ...
      test("should show hover and click labels at any unclustered zoom and hide them while clustered", async ({
        page,
      }) => {
        // arrange
        await mockLuxembourgOverlayStyle(page);
        ...
        await movePointerOffTrain(page);
        await expect
          .poll(async () => (await readTrainInteractionSnapshot(page)).hoveredRouteLabelCount, { timeout: 20_000 })
          .toBe(0);
        await expect
          .poll(async () => (await readTrainInteractionSnapshot(page)).hoveredOperatorLabelCount, { timeout: 20_000 })
          .toBe(0);
